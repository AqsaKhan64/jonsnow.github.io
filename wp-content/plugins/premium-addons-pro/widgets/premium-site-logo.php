...
class Premium_Site_Logo extends Widget_Base {
	...
	private function add_logo_content_controls( $draw_icon ) {
		...
		$this->add_control(
			'image',
			array(
				'label'       => esc_html__( 'Image', 'premium-addons-pro' ),
		...
		$this->add_control(
			'image_2x',
			array(
				'label'       => esc_html__( 'Retina Image', 'premium-addons-pro' ),
				'type'        => Controls_Manager::MEDIA,
				'media_types' => array( 'image' ),
				'dynamic'     => array( 'active' => true ),
				'condition'   => array(
		...
		$this->add_control(
			'svg',
			array(
				'label'       => esc_html__( 'Choose SVG Image', 'premium-addons-pro' ),
				'type'        => Controls_Manager::MEDIA,
				...
				<?php if ( ! empty( $settings['logo_heading'] ) || ! empty( $settings['logo_sub_heading'] ) ) : ?>
					<div class="premium-logo-text-wrapper">
						<?php if ( ! empty( $settings['logo_heading'] ) ) : ?>
							<span class="premium-logo-heading"> <?php echo esc_html( $settings['logo_heading'] ); ?> </span>
						<?php endif; ?>
						<?php if ( ! empty( $settings['logo_sub_heading'] ) ) : ?>
							<span class="premium-logo-sub-heading"><?php echo esc_html( $settings['logo_sub_heading'] ); ?> </span>
						<?php endif; ?>
					</div>
				<?php endif; ?>
			</div>
			...
			if ( ! empty( $settings['svg']['url'] ) ) :
				...
				if ( ! is_wp_error( $svg_code ) ) :
					...
					<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'svg_container' ) ); ?> >
						<?php echo wp_remote_retrieve_body( $svg_code ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</div>
					<?php
				endif;
			endif;
